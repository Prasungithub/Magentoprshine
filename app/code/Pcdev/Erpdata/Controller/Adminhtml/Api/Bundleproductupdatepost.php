...
class Bundleproductupdatepost extends \Magento\Backend\App\Action
{
	...
	protected $_publicActions = ['bundleproductupdatepost'];
    ...
    {		
		...
		$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/bndl-prod-import.log');
		$logger = new \Zend_Log();
		...
		if(isset($params['requestbundlesync']) && $params['requestbundlesync'] == 1){
			...
			if($selectAllitems_count > 0){
				...
				//for($c = 1; $c <= $loopcount;){
					...
					echo 'Updated row no.: ' .$updatedrecordcounter;
					echo '<br/>';
					$start = $start + ($rowno - 1);
				//	$c = $c + 1;
				//}
