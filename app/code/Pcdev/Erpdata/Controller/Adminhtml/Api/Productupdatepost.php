...
class Productupdatepost extends \Magento\Backend\App\Action
{
	...
	protected $_publicActions = ['productupdatepost'];
    ...
    {		
		...
		$writer = new \Zend_Log_Writer_Stream(BP . '/var/log/prproderplog.log');
		$logger = new \Zend_Log();
		...
		if(isset($params['requestsync']) && $params['requestsync'] == 1){
			
			$selectAllitems = $this->_erpviewblock->getAllItemsSku();
			$selectAllitems_count = count($selectAllitems);
			if($selectAllitems_count > 0){				
				$rowno = 40;
				$loopcount = round($selectAllitems_count/$rowno);
				$start = 0;
				try{
				for($c = 1; $c <= $loopcount;){
					try{
						
					$allRecordsResult = $this->_erpviewblock->getAllRecordswithLimit($start,$rowno);				
					if(count($allRecordsResult) > 0){						
						foreach($allRecordsResult as $allRecord){
						try{
						...
						if($allRecord['ispresent'] == '1'){	
							...
							if(empty($product['attributeSetId'])){
								...
								$logger->info('Error in Quantity in sku: '. $mismatchdata_sku);
							...
							if($sku){
									...
									foreach($product['pricearray'] as $priceVal){
										...
										try
										{
											...
											if ($stockItem->getQty() != $product['quantity'])
											{
												...
												$logger->info('Tushar simple: '. $associatedProductSku."|".$quantity);
											}else{
												$logger->info('Tushar simple else: '. $associatedProductSku."|".$stockItem->getQty()."|".$product['quantity']);
											}
										} catch (\Magento\Framework\Exception\NoSuchEntityException $e){
											$logger->info('Error in Quantity in sku: '. $associatedProductSku);
											$logger->info($e->getMessage());
								...
								if(count($associatedProductIds) > 0){									
									...
									try{	
										...
										$logger->info('Child Item Not Found '.$default_child_product);
										$logger->info($e->getMessage());
									}
									...
									if(isset($options) && count($options) > 0){
										...
										if ($exist == '0') {	
											 try {
												...
												$logger->info('Error in Custom Option Packing in sku: '. $configurableProductId);
									...
									try
									{
										...
										if ($conf_stockItem->getQty() != $product['quantity'])
										{
											...
											$logger->info('Tushar configurable: '. $sku."|".$quantity);
										}else{
											$logger->info('Tushar configurable else: '. $sku."|".$conf_stockItem->getQty()."|".$product['quantity']);
										}
									} catch (\Magento\Framework\Exception\NoSuchEntityException $e){
										$logger->info('Error in Quantity in sku: '. $sku);
										$logger->info($e->getMessage());
					...
					$start_rowno = $start+1;
					$end_rowno = $start+$rowno;
					echo 'Updated row no.: ' .$start_rowno .' to '. $end_rowno;
					$start = $start + ($rowno - 1);
					$c = $c + 1;
					}catch(\Exception $e){
						$logger->info($e->getMessage());
	...
	public function getAttributeSetId($attributeSetName)
	{
		...
		$AttributeSetArr = array('DEFAULT'=>4,'BAYARA'=>9,'NUTS & SEEDS'=>10,'DRIED FRUITS & DATES'=>11,'SPICES & SEASONING'=>12,'PULSES GRAINS'=>13, 'PULSES & GRAINS'=>13);
